...
def get_tavily_api_keys() -> List[str]:
    """Get Tavily API keys from environment.

    Supports multiple keys via comma-separated TAVILY_API_KEYS or
    SCOUT_TAVILY_API_KEYS, with single-key fallbacks for compatibility.
    """
...
def get_tavily_api_key() -> Optional[str]:
    """Get first available Tavily API key (legacy compat)."""
...
def search_tavily(query: str, api_key: str, topic: str = "general",
                  ...
                  days: Optional[int] = None, key_label: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.warning(f"Tavily search error for '{query}' via {key_label or 'key'}: HTTP {e.code}")
        return {"query": query, "status": "error", "total": 0, "results": [], "error": f"HTTP {e.code}", "key_label": key_label}
    except Exception as e:
        logging.warning(f"Tavily search error for '{query}' via {key_label or 'key'}: {e}")
        return {"query": query, "status": "error", "total": 0, "results": [], "error": str(e), "key_label": key_label}


def search_topic_tavily(topic: Dict[str, Any], api_keys: List[str], days: Optional[int] = None) -> Dict[str, Any]:
    ...
    for index, query in enumerate(queries):
        ...
        for offset in range(len(api_keys)):
            key_index = (index + offset) % len(api_keys)
            candidate = search_tavily(
                query,
                api_keys[key_index],
                topic="news",
                days=days,
            ...
            error_text = str(candidate.get("error", "")).lower()
            if offset + 1 >= len(api_keys):
                break
            if not any(token in error_text for token in ("http 401", "http 403", "http 429")):
                continue
        ...
        query_stats.append({
            ...
            "status": search_result["status"],
            "count": search_result["total"],
            "key": search_result.get("key_label"),
        })
        if search_result["status"] == "ok":
            for result in search_result["results"]:
                combined_text = f"{result['title']} {result['snippet']}"
...
def apply_search_targets(topics: List[Dict[str, Any]], search_targets_path: Optional[Path], logger: logging.Logger) -> List[Dict[str, Any]]:
    """Override topic queries at runtime using an explicit search target file."""
    if not search_targets_path:
        return topics
    if not search_targets_path.exists():
        raise FileNotFoundError(f"search targets file not found: {search_targets_path}")

    payload = json.loads(search_targets_path.read_text(encoding='utf-8'))
    if isinstance(payload, list):
        targets = payload
    else:
        targets = payload.get('search_targets') or payload.get('topics') or []

    topic_map = {topic['id']: topic for topic in topics}
    selected_topics = []

    for entry in targets:
        topic_id = entry.get('topic') or entry.get('id')
        if not topic_id:
            logger.warning('Skipping search target without topic/id')
            continue
        if topic_id not in topic_map:
            logger.warning(f"Skipping unknown search target topic: {topic_id}")
            continue

        topic = json.loads(json.dumps(topic_map[topic_id]))
        ...
        if 'must_include' in entry:
            search['must_include'] = entry.get('must_include', [])
        if 'exclude' in entry:
            search['exclude'] = entry.get('exclude', [])
        if 'twitter_queries' in entry:
            search['twitter_queries'] = entry.get('twitter_queries', [])
        topic['search'] = search
        selected_topics.append(topic)

    logger.info(f"Using explicit search targets for {len(selected_topics)} topic(s) from {search_targets_path}")
...
Examples:
    ...
    parser.add_argument(
        "--search-targets",
        type=Path,
        help="Optional JSON file that defines the exact topics and queries to search for this run"
    )
    ...
    try:
        ...
        topics = apply_search_targets(topics, args.search_targets, logger)
        ...
        if use_tavily:
            logger.info(f"Using Tavily Search API for {len(topics)} topics ({len(tavily_keys)} key(s) configured)")
            ...
            for topic in topics:
                ...
                result = search_topic_tavily(topic, tavily_keys, days=tavily_days)
            ...
            output = {
                ...
                "api_used": "tavily",
                "api_keys_configured": len(tavily_keys),
